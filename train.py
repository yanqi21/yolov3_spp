...
def train(hyp):
    ...
    #先训练三个预测器，再训练除darknet53之外的权重效果会更好
    ...
    # optimizer
    pg = [p for p in model.parameters() if p.requires_grad]        #如果为true则没有被冻结
    ...
    # coco = None
    coco = get_coco_api_from_dataset(val_dataset)#遍历验证集
...
if __name__ == '__main__':
    ...
    parser.add_argument('--notest', action='store_true', help='only test final epoch')#default false,test every epoch
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training')
    parser.add_argument('--weights', type=str, default='../weights/yolov3-spp-ultralytics-512.pt',
                        help='initial weights path')#change weights
    parser.add_argument('--name', default='', help='renames results.txt to results_name.txt if supplied')
    parser.add_argument('--device', default='cuda:0', help='device id (i.e. 0 or 0,1 or cpu)')
    parser.add_argument('--single-cls', action='store_true', help='train as single-class dataset')
    #是否冻结网络的部分权重，true->训练三个预测器,false->冻结darknet53
